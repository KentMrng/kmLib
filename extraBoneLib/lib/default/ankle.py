...
ankle_driven_info = [
    {
        'driver': 'ankleFront_{}0_jnt',
        'behavior': 'leg_L0_end_jnt',
        'drivenkeys': [
            {
                'driver_attr': 'rz',
                'driven_attr': 'ty',
                'driver_value': [45, 0, -45],
                'driven_value': [-1, 0, -0.2],
                'option': {'pre': 0}
            }
        ]
    },
    {
        'driver': 'ankleBack_{}0_jnt',
        'behavior': 'leg_L0_end_jnt',
        'drivenkeys': [
            {
                'driver_attr': 'rz',
...
parts_info {
    'base_joint': 'leg_{}0_end_jnt',
    'joint_radius': 1.4,
    'ankle_driven_info': ankle_driven_info
}
...
class Main(libParts.Main):
    ...
    def execute(self):
        ...
        ankle_driven_info = self.parts_info.get('ankle_driven_info')

        for side in 'LR':
            joint = base_joint.format(side)
            bend_joint = node.create_bend_joint(joint, name=joint.replace('jnt', 'bendJnt'))
            blend_joint = node.add_blended_joint(bend_joint, blend=0.25, name=joint.replace('jnt', 'bendBldJnt'))

            joints = node.driven_joint_key(ankle_driven_info, side, joint, blend_joint)
